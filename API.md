...
1. 환경설정시, CRA 사용
2. git clone 후, npm install & npm start 명령어를 통해 정상동작하도록 구현
3. 함수형 컴포넌트 사용
4. UI는 평가에 영향x 자연스러운 형태
5. README.md 작성 필수
6. 기능 구현에 직접적으로 연관된 라이브러리 사용 X (e.g. react-query)
   > 사용하지 않더라도 package.json의 devDependencies,에 기재되있을 시, 사용한 것으로 간주
7. 사용가능 라이브러리 목록

```markdown
1.  React Router
2.  HTTP Client 라이브러리(Axios 등)
3.  스타일링 관련 라이브러리(Sass, Styled Components, Emotion, tailwind 등)
4.  아이콘 등 UI 관련 라이브러리(Font-Awesome, React-Icons, Bootstrap 등)
5.  기능과 직접적인 연관이 없는 설정관련 라이브러리(craco, dotenv, typescript, testing library 등)
```

8. 페이지별로 요구되는 경로는 도메인 뒤의 URL뒤에 바로 이어지도록 설정
...
- 기능이 정상 동작하지 않는 예시: 새로고침하면 404 에러 페이지 표출 ("/" URL이 아닌 "/signup"등의 경로로 바로 접속할 경우 404 에러 페이지 표출 등)

# 과제

- 가이드라인에 명시된 `test-id` 속성이 제대로 부여되지 않은 경우 구현된 것으로 판단 X
- 가이드라인에 명시된 사항 외에는 자유
